...
## 2024-07-04 - [Preventing Layout Thrashing in renderState]
**Learning:** High-frequency UI updates in `renderState` cause layout thrashing because it repeatedly accesses the DOM for every stat even if the value hasn't changed.
**Action:** Caching primitive values (`loc`, `tokens`, `stat.val`) in an in-memory `lastRenderState` object allows for fast early returns, bypassing redundant DOM manipulations.
