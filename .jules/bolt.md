...
## 2024-05-19 - [Preventing ReDoS in JSON extraction]
**Learning:** Using greedy regex patterns like `/{[\s\S]*"loc"[\s\S]*}/g` to extract JSON payloads from text nodes causes catastrophic backtracking (ReDoS) on large strings, severely impacting performance.
**Action:** Avoid regex for this use case. Instead, prefer O(N) string operations like `indexOf('{')` and `lastIndexOf('}')` to safely isolate and extract content boundaries.
