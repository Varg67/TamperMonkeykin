...
## 2024-05-19 - [Caching Primitives to Prevent UI Thrashing]
**Learning:** High-frequency UI updates in `renderState` can cause layout thrashing when redundant DOM updates occur.
**Action:** Cache primitive values (`stat.val`, `loc`, `tokens`) in an in-memory `lastRenderState` object and use early returns to safely bypass redundant DOM iterations and prevent layout reflows without repeatedly reading from the DOM.
