...
## 2024-05-24 - Fix DOM API Key Exposure
**Vulnerability:** API keys were populated directly into the DOM input `.value` attribute during initialization, exposing them to potentially malicious host page scripts.
**Learning:** Never inject sensitive data into DOM attributes in userscripts, as this exposes them to the host page environment.
**Prevention:** Store credentials securely in memory/storage, use a dummy mask string (e.g., `••••••••••••••••`) in the DOM for visual feedback, and fallback to the in-memory key when the mask is present.
