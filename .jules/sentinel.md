...
## 2026-07-05 - Prevent ReDoS in JSON parsing
**Vulnerability:** Regular Expression Denial of Service (ReDoS) from a greedy regex pattern (/{[\s\S]*"loc"[\s\S]*}/g) parsing untrusted long strings.
**Learning:** Greedy regex patterns cause catastrophic backtracking. String boundaries and content must be safely isolated.
**Prevention:** Use O(N) string operations like indexOf('{') and lastIndexOf('}') to safely isolate JSON boundaries instead of complex regular expressions.
