...
## 2026-07-08 - Prevent ReDoS in JSON extraction
**Vulnerability:** Greedy regular expression (`/[\s\S]*"loc"[\s\S]*/g`) used to extract untrusted JSON payloads could lead to Regular Expression Denial of Service (ReDoS) freezing the UI.
**Learning:** Parsing untrusted LLM payload strings with greedy wildcard regexes is computationally dangerous.
**Prevention:** Use O(N) string operations like `indexOf('{')` and `lastIndexOf('}')` to safely isolate boundaries, then validate the extracted string with `.includes()` before parsing.
