...
## 2026-04-13 - Fix DOM attribute secret injection
**Vulnerability:** Injecting sensitive data (like API keys) directly into DOM attributes (e.g., input `.value`) during initialization.
**Learning:** This exposes them to malicious scripts on the host page. The DOM is accessible to all extensions and scripts running on a page.
**Prevention:** Store credentials securely in memory or extension storage, use placeholder text for UI feedback, and read/clear inputs only during active user interactions.
