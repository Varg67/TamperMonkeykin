...
## 2024-05-20 - Fix ReDoS in JSON Payload Parsing
**Vulnerability:** A greedy regex `/{[\s\S]*"loc"[\s\S]*}/g` was used to extract JSON payloads from DOM text nodes, which can cause catastrophic backtracking (ReDoS) and freeze the browser.
**Learning:** The vulnerability existed because regex was used to match complex string boundaries within long user-generated text nodes instead of O(N) string boundary operations.
**Prevention:** Avoid greedy regex for JSON block isolation. Prefer `indexOf('{')` and `lastIndexOf('}')` to define boundaries safely, validating the substring contains target keywords.
