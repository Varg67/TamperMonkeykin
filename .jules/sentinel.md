...
## 2026-06-06 - Prevent ReDoS in Payload Extraction
**Vulnerability:** Greedy regex pattern `/{[\s\S]*"loc"[\s\S]*}/g` used for JSON extraction caused catastrophic backtracking on long strings without a closing brace.
**Learning:** Regular Expression Denial of Service (ReDoS) can freeze the extension when processing untrusted, malformed text nodes from the host page.
**Prevention:** Always use safe O(N) string boundary functions like `indexOf('{')` and `lastIndexOf('}')` combined with `includes()` checks when extracting serialized objects from larger text blocks instead of regex.
