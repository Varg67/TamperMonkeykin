...
## 2024-04-07 - API Key DOM Exposure in Userscripts
**Vulnerability:** API keys were directly injected into DOM input values (`input.value = savedKeys.k`) during UI initialization, exposing them to potentially malicious host page scripts.
**Learning:** Injecting sensitive data into DOM attributes in Tampermonkey scripts exposes the data to the host environment. Credentials must remain in memory/storage.
**Prevention:** Use a dummy mask string (e.g., `••••••••••••••••`) in the UI, and implement a helper function on save that falls back to the in-memory key if the mask is still present.
