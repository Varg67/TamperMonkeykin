...
## 2024-06-11 - DOM Attribute Secret Exposure in Userscripts
**Vulnerability:** API keys were injected directly into input.value, making them readable by malicious scripts on the host page.
**Learning:** Userscripts share the DOM with the host page. Injecting credentials into the DOM exposes them.
**Prevention:** Store credentials in memory/storage, and use a dummy mask string (e.g. ••••••••••••••••) in the UI.
