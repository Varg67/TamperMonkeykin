...
## 2024-04-07 - DOM Extraction Vulnerability
**Vulnerability:** Injecting sensitive data (API keys) directly into DOM element attributes like input `.value`.
**Learning:** Malicious host page scripts can easily extract credentials from DOM nodes, so UI elements must only show dummy mask strings (e.g., `••••••••••••••••`) while real keys reside securely in memory.
**Prevention:** Use placeholder or mask strings in the DOM for loaded credentials and define helper functions to fetch in-memory values when the mask is present.
