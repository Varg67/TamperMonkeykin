...
## 2026-06-24 - API Key DOM Exposure in Userscripts
**Vulnerability:** Injecting sensitive data like API keys directly into DOM elements' `.value` during initialization exposes them to malicious host page scripts.
**Learning:** Userscripts share the DOM with the host page. Any script on the host page can query input elements and extract their plaintext `.value`.
**Prevention:** Store credentials securely in memory/storage, and populate `.value` with a dummy mask string (e.g., `••••••••••••••••`). Use a helper to map the mask back to the real value when needed.
