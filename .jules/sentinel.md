...
## 2026-06-15 - DOM Input Masking for API Keys
**Vulnerability:** API keys injected directly into DOM `input.value` in userscript, exposing them to host page XSS.
**Learning:** Storing keys in DOM attributes in a Tampermonkey script violates isolation because the host page can easily scrape them.
**Prevention:** Always use a dummy mask in the DOM (`••••••••••••••••`) while retaining the real values in JS scope/GM_getValue.
