...
**Vulnerability:** API keys were injected directly into input DOM attributes (.value) during initialization.
**Learning:** Injecting secrets into the DOM in userscripts exposes them to malicious host page scripts that can read input values.
**Prevention:** Use a dummy mask string (e.g., `••••••••••••••••`) in the DOM and keep the actual keys in an isolated memory state variable, swapping it back when saving or making requests. Sanitize legacy stored values during load.
