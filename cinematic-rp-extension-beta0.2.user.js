...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        ['k', 'c', 'g'].forEach(key => {
            ...
            if (savedKeys[key]) {
                el.placeholder = '(Saved Key)';
            }
            el.addEventListener('input', () => {
                if (el.value === '' && savedKeys[key]) {
                    el.placeholder = 'Cleared';
                }
            });
        });
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            ['k', 'c', 'g'].forEach(key => {
                ...
                if (el.value) {
                    ...
                    el.placeholder = '(Saved Key)';
                    el.value = '';
                } else if (el.placeholder === 'Cleared') {
                    gameState.apiKeys[key] = '';
                    el.placeholder = key === 'k' ? 'sk-kindroid...' : key === 'c' ? 'Kin ID...' : 'AIzaSy...';
                }
            });
