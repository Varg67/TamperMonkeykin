...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const getActualKey = (type, inputVal) => inputVal === MASK ? (gameState.apiKeys[type] || '') : inputVal;
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getActualKey('k', rawK));
            gameState.apiKeys.c = sanitizeKey(getActualKey('c', rawC));
            gameState.apiKeys.g = sanitizeKey(getActualKey('g', rawG));

            GM_setValue('knd_rp_keys', JSON.stringify(gameState.apiKeys));

            // Re-mask if user entered new keys
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getActualKey(type, rawTargetVal).replace(/[\r\n]/g, '').trim();
            const keyK = getActualKey('k', rawKeyK).replace(/[\r\n]/g, '').trim();
