...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const DUMMY_MASK = '••••••••••••••••';
        const getUnmaskedValue = (id, key) => {
            const val = document.getElementById(id).value;
            return val === DUMMY_MASK ? (gameState.apiKeys[key] || '') : val;
        };

        document.getElementById('inp-api-k').value = savedKeys.k ? DUMMY_MASK : '';
        document.getElementById('inp-api-c').value = savedKeys.c ? DUMMY_MASK : '';
        document.getElementById('inp-api-g').value = savedKeys.g ? DUMMY_MASK : '';
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getUnmaskedValue('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getUnmaskedValue('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getUnmaskedValue('inp-api-g', 'g'));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = (targetValRaw === DUMMY_MASK ? (gameState.apiKeys[type] || '') : targetValRaw).replace(/[\r\n]/g, '').trim();

            const keyKRaw = document.getElementById('inp-api-k').value;
            const keyK = (keyKRaw === DUMMY_MASK ? (gameState.apiKeys.k || '') : keyKRaw).replace(/[\r\n]/g, '').trim();
