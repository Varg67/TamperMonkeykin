...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getSafeInput('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getSafeInput('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getSafeInput('inp-api-g', 'g'));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getSafeInput(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getSafeInput('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
