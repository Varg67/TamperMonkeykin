...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const applyMask = (id, key) => { document.getElementById(id).value = key ? MASK : ''; };
        const getRealValue = (id, memKey) => {
            const val = document.getElementById(id).value;
            return val === MASK ? memKey : sanitizeKey(val);
        };

        applyMask('inp-api-k', gameState.apiKeys.k);
        applyMask('inp-api-c', gameState.apiKeys.c);
        applyMask('inp-api-g', gameState.apiKeys.g);

        document.getElementById('knd-save-keys').addEventListener('click', () => {
            // Security: Mask API keys in DOM to prevent extraction by host page scripts
            gameState.apiKeys.k = getRealValue('inp-api-k', gameState.apiKeys.k);
            gameState.apiKeys.c = getRealValue('inp-api-c', gameState.apiKeys.c);
            gameState.apiKeys.g = getRealValue('inp-api-g', gameState.apiKeys.g);
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getRealValue(`inp-api-${type}`, gameState.apiKeys[type]);
            const keyK = getRealValue('inp-api-k', gameState.apiKeys.k);
            ...
            try {
                ...
                if(success) {
                    ...
                    applyMask(`inp-api-${type}`, targetVal);
                }
