...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getInputValue('inp-api-k', gameState.apiKeys.k));
            gameState.apiKeys.c = sanitizeKey(getInputValue('inp-api-c', gameState.apiKeys.c));
            gameState.apiKeys.g = sanitizeKey(getInputValue('inp-api-g', gameState.apiKeys.g));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getInputValue(`inp-api-${type}`, gameState.apiKeys[type]).replace(/[\r\n]/g, '').trim();
            const keyK = getInputValue('inp-api-k', gameState.apiKeys.k).replace(/[\r\n]/g, '').trim();
            ...
            try {
                ...
                if(success) {
                    ...
                    document.getElementById(`inp-api-${type}`).value = targetVal ? '••••••••••••••••' : '';
                }
