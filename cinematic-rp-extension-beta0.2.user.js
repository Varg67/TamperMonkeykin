...
(function() {
    ...
    // ⚡ Bolt Optimization: Cache previous primitive values to prevent unnecessary DOM updates.
    // Impact: Avoids heavy layout reflows/thrashing by ensuring we only touch the DOM if a value actually changed.
    const lastRenderState = { loc: null, tokens: null, stats: {} };

    const renderState = () => {
        // Fast-path for loc and tokens
        ...
        Object.keys(gameState.stats).forEach(key => {
            ...
            // Early return to skip modifying DOM block styles if the numerical stat hasn't changed.
