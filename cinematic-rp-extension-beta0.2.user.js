...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const mask = '••••••••••••••••';
        const getRealValue = (inputId, type) => {
            const val = document.getElementById(inputId).value;
            return val === mask && gameState.apiKeys[type] ? gameState.apiKeys[type] : val;
        };
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getRealValue('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getRealValue('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getRealValue('inp-api-g', 'g'));

            // Re-apply mask after save
        ...
        const pingAPI = async (type) => {
            ...
            const mask = '••••••••••••••••';
            const getRealValuePing = (inputId, t) => {
                 const val = document.getElementById(inputId).value;
                 return val === mask && gameState.apiKeys[t] ? gameState.apiKeys[t] : val;
            };

            const targetVal = getRealValuePing(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getRealValuePing('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
