...
(function() {
    ...
    // ==========================================
    const maskString = '••••••••••••••••';
    const getMaskedValue = (inputId, key) => {
        const val = document.getElementById(inputId).value;
        return val === maskString ? (gameState.apiKeys[key] || '') : val;
    };
    ...
    const initToolsLogic = () => {
        ...
        document.getElementById('inp-api-k').value = savedKeys.k ? maskString : '';
        document.getElementById('inp-api-c').value = savedKeys.c ? maskString : '';
        document.getElementById('inp-api-g').value = savedKeys.g ? maskString : '';
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getMaskedValue('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getMaskedValue('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getMaskedValue('inp-api-g', 'g'));

            document.getElementById('inp-api-k').value = gameState.apiKeys.k ? maskString : '';
            document.getElementById('inp-api-c').value = gameState.apiKeys.c ? maskString : '';
            document.getElementById('inp-api-g').value = gameState.apiKeys.g ? maskString : '';
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getMaskedValue(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getMaskedValue('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
