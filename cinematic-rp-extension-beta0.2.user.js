...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const getUnmaskedValue = (inputId, keyName) => {
            const val = document.getElementById(inputId).value;
            return val === MASK ? (gameState.apiKeys[keyName] || '') : val;
        };
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getUnmaskedValue(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getUnmaskedValue('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
