...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const getSafeValue = (id, key) => {
            const val = document.getElementById(id).value;
            if (val === MASK) return gameState.apiKeys[key] || '';
            return val;
        };
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getSafeValue('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getSafeValue('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getSafeValue('inp-api-g', 'g'));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getSafeValue(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getSafeValue('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
            ...
            try {
                ...
                if(success) {
                    ...
                    document.getElementById(`inp-api-${type}`).value = targetVal ? MASK : '';
                }
