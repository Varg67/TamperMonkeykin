...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        // Security: Sanitize legacy keys and initialize state
        gameState.apiKeys.k = sanitizeKey(savedKeys.k);
        gameState.apiKeys.c = sanitizeKey(savedKeys.c);
        gameState.apiKeys.g = sanitizeKey(savedKeys.g);

        // Security: Mask API keys in DOM to prevent exposure to malicious host scripts
        const MASK = '••••••••••••••••';
        const getInputValue = (id, key) => {
            const val = document.getElementById(id).value;
            return val === MASK ? gameState.apiKeys[key] : sanitizeKey(val);
        };
