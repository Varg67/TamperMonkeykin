...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const DUMMY_MASK = '••••••••••••••••';
        const getUnmaskedVal = (inputId, type) => {
            const val = document.getElementById(inputId).value;
            return val === DUMMY_MASK ? (gameState.apiKeys[type] || '') : val;
        };
        const maskInput = (inputId, type) => {
            document.getElementById(inputId).value = gameState.apiKeys[type] ? DUMMY_MASK : '';
        };

        maskInput('inp-api-k', 'k');
        maskInput('inp-api-c', 'c');
        maskInput('inp-api-g', 'g');
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getUnmaskedVal('inp-api-k', 'k'));
            gameState.apiKeys.c = sanitizeKey(getUnmaskedVal('inp-api-c', 'c'));
            gameState.apiKeys.g = sanitizeKey(getUnmaskedVal('inp-api-g', 'g'));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getUnmaskedVal(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getUnmaskedVal('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
