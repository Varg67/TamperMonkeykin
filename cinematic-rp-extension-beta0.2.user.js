...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        // Security: Sanitize legacy keys and prevent DOM exposure
        ...
        const resolveKey = (inputId, currentKey) => {
            const val = document.getElementById(inputId).value;
            return val === mask ? currentKey : val;
        };

        document.getElementById('knd-save-keys').addEventListener('click', () => {
            gameState.apiKeys.k = sanitizeKey(resolveKey('inp-api-k', gameState.apiKeys.k));
            gameState.apiKeys.c = sanitizeKey(resolveKey('inp-api-c', gameState.apiKeys.c));
            gameState.apiKeys.g = sanitizeKey(resolveKey('inp-api-g', gameState.apiKeys.g));
