...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const getUnmaskedValue = (type, val) => val === MASK ? (gameState.apiKeys[type] || '') : val;
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            const rawK = document.getElementById('inp-api-k').value;
            const rawC = document.getElementById('inp-api-c').value;
            const rawG = document.getElementById('inp-api-g').value;

            gameState.apiKeys.k = sanitizeKey(getUnmaskedValue('k', rawK));
            gameState.apiKeys.c = sanitizeKey(getUnmaskedValue('c', rawC));
            gameState.apiKeys.g = sanitizeKey(getUnmaskedValue('g', rawG));

            // Re-apply mask after saving if it was newly typed
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getUnmaskedValue(type, rawTargetVal).replace(/[\r\n]/g, '').trim();

            const rawKeyK = document.getElementById('inp-api-k').value;
            const keyK = getUnmaskedValue('k', rawKeyK).replace(/[\r\n]/g, '').trim();
            ...
            try {
                ...
                if(success) {
                    ...
                    document.getElementById(`inp-api-${type}`).value = MASK;
                }
