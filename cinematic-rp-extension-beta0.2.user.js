...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const MASK = '••••••••••••••••';
        const getUnmaskedValue = (inputId, inMemoryKey) => {
            const val = document.getElementById(inputId).value;
            return val === MASK ? (inMemoryKey || '') : val;
        };

        // Security: Mask keys in DOM to prevent exposure to host page scripts
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getUnmaskedValue('inp-api-k', gameState.apiKeys.k));
            gameState.apiKeys.c = sanitizeKey(getUnmaskedValue('inp-api-c', gameState.apiKeys.c));
            gameState.apiKeys.g = sanitizeKey(getUnmaskedValue('inp-api-g', gameState.apiKeys.g));

            // Re-apply masks if they successfully saved something
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getUnmaskedValue(`inp-api-${type}`, gameState.apiKeys[type]).replace(/[\r\n]/g, '').trim();
            const keyK = getUnmaskedValue('inp-api-k', gameState.apiKeys.k).replace(/[\r\n]/g, '').trim();
