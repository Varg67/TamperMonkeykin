...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        // Security: Use mask string to prevent exposing keys to host page DOM
        const MASK = '••••••••••••••••';
        const getRealValue = (elId, memKey) => {
            const val = document.getElementById(elId).value;
            return val === MASK ? (gameState.apiKeys[memKey] || '') : val;
        };
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getRealValue(`inp-api-${type}`, type).replace(/[\r\n]/g, '').trim();
            const keyK = getRealValue('inp-api-k', 'k').replace(/[\r\n]/g, '').trim();
