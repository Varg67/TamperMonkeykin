...
(function() {
    ...
    const renderState = () => {
        ...
        if (domCache.locText.textContent !== newLocText) {
            domCache.locText.textContent = newLocText;
        }
        const newTokensText = gameState.tokens + 'T';
        if (domCache.tokens.textContent !== newTokensText) {
        ...
        Object.keys(gameState.stats).forEach(key => {
            ...
            if (stat.visualHash !== visualHash) {
                stat.visualHash = visualHash;
                for (let i = 0; i < 10; i++) {
