                ...
                if(mutation.addedNodes.length > 0) {
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === 1 || node.nodeType === 3) {
                            ...
                            // Optimization: Replaced greedy regex with O(N) string boundary operations
                            // to prevent catastrophic backtracking and significant main thread blocking
                            // when parsing potentially massive text nodes inside the hot observer path.
