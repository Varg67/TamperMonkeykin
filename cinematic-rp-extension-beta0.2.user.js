...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        const maskString = '••••••••••••••••';
        const getSafeInputVal = (type) => {
            const val = document.getElementById(`inp-api-${type}`).value;
            if (val === maskString) return gameState.apiKeys[type] || '';
            return val;
        };
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            gameState.apiKeys.k = sanitizeKey(getSafeInputVal('k'));
            gameState.apiKeys.c = sanitizeKey(getSafeInputVal('c'));
            gameState.apiKeys.g = sanitizeKey(getSafeInputVal('g'));
        ...
        const pingAPI = async (type) => {
            ...
            const targetVal = getSafeInputVal(type).replace(/[\r\n]/g, '').trim();
            const keyK = getSafeInputVal('k').replace(/[\r\n]/g, '').trim();
