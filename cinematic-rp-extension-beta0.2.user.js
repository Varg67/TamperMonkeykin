...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        // Security: Do not expose raw API keys in DOM .value attributes
        ...
        const resolveInputValue = (inputId, currentKey) => {
            const val = document.getElementById(inputId).value;
            return val === MASK ? currentKey : sanitizeKey(val);
        };

        document.getElementById('knd-save-keys').addEventListener('click', () => {
            gameState.apiKeys.k = resolveInputValue('inp-api-k', gameState.apiKeys.k);
            gameState.apiKeys.c = resolveInputValue('inp-api-c', gameState.apiKeys.c);
            gameState.apiKeys.g = resolveInputValue('inp-api-g', gameState.apiKeys.g);
        ...
        const pingAPI = async (type) => {
            ...
            // Security: Resolve masked input value to prevent header injection
            const targetVal = resolveInputValue(`inp-api-${type}`, gameState.apiKeys[type]);
            const keyK = resolveInputValue('inp-api-k', gameState.apiKeys.k);
