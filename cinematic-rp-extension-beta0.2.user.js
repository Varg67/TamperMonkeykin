...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        if (savedKeys.k) document.getElementById('inp-api-k').placeholder = '*** (Stored)';
        if (savedKeys.c) document.getElementById('inp-api-c').placeholder = '*** (Stored)';
        if (savedKeys.g) document.getElementById('inp-api-g').placeholder = '*** (Stored)';
        ...
        document.getElementById('knd-save-keys').addEventListener('click', () => {
            ...
            if (inputK.value) { gameState.apiKeys.k = sanitizeKey(inputK.value); inputK.value = ''; inputK.placeholder = '*** (Stored)'; }
            if (inputC.value) { gameState.apiKeys.c = sanitizeKey(inputC.value); inputC.value = ''; inputC.placeholder = '*** (Stored)'; }
            if (inputG.value) { gameState.apiKeys.g = sanitizeKey(inputG.value); inputG.value = ''; inputG.placeholder = '*** (Stored)'; }
        ...
        const pingAPI = async (type) => {
            ...
            if (!targetVal) targetVal = (gameState.apiKeys[type] || '').replace(/[\r\n]/g, '').trim();

            let keyK = document.getElementById('inp-api-k').value.replace(/[\r\n]/g, '').trim();
            if (!keyK) keyK = (gameState.apiKeys.k || '').replace(/[\r\n]/g, '').trim();
