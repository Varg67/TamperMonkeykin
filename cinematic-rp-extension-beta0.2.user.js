...
(function() {
    ...
    const renderState = () => {
        ...
        if (renderCache.loc !== locStr) {
            domCache.locText.textContent = locStr;
            renderCache.loc = locStr;
        }

        const tokensStr = gameState.tokens + 'T';
        if (renderCache.tokens !== tokensStr) {
            domCache.tokens.textContent = tokensStr;
            renderCache.tokens = tokensStr;
        }
        ...
        Object.keys(gameState.stats).forEach(key => {
            ...
            if (renderCache[`val-${key}`] !== valStr) {
                domCache[`val-${key}`].textContent = valStr;
                renderCache[`val-${key}`] = valStr;
            }

            if (key === 'pleasure') {
                const displayStr = stat.val > 0 ? 'flex' : 'none';
                if (renderCache.pleasureDisplay !== displayStr) {
                    domCache.pleasureStat.style.display = displayStr;
                    renderCache.pleasureDisplay = displayStr;
                }
            }
            ...
            if (renderCache[`blocks-${key}`] !== stateHash) {
                for (let i = 0; i < 10; i++) {
                ...
                renderCache[`blocks-${key}`] = stateHash;
            }
        });
    };
