...
(function() {
    ...
    const initToolsLogic = () => {
        ...
        // Security: Explicitly sanitize keys loaded from storage
        gameState.apiKeys = {
        ...
        const MASK = '••••••••••••••••';
        const getMaskedVal = (val) => val ? MASK : '';

        // Security: Mask API keys in DOM to prevent exposure to host page scripts
        document.getElementById('inp-api-k').value = getMaskedVal(gameState.apiKeys.k);
        document.getElementById('inp-api-c').value = getMaskedVal(gameState.apiKeys.c);
        document.getElementById('inp-api-g').value = getMaskedVal(gameState.apiKeys.g);

        document.getElementById('knd-save-keys').addEventListener('click', () => {
            const getNewValue = (inputId, currentKey) => {
                const val = document.getElementById(inputId).value;
                return val === MASK ? currentKey : sanitizeKey(val);
            };
        ...
        const pingAPI = async (type) => {
            ...
            const MASK = '••••••••••••••••';
            // Security: Prevent header injection by removing newlines and resolve masked values
            ...
            const targetVal = rawTargetVal === MASK ? gameState.apiKeys[type] : rawTargetVal;
            const keyK = rawKeyK === MASK ? gameState.apiKeys.k : rawKeyK;
