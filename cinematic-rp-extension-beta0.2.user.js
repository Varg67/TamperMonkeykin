    ...
    const initToolsLogic = () => {
        ...
        const setPlaceholder = (id, val) => {
            ...
            if (val) {
                ...
                // If cleared, restore default placeholder based on ID
                el.value = '';
                if(id === 'inp-api-k') el.placeholder = 'sk-kindroid...';
                if(id === 'inp-api-c') el.placeholder = 'Kin ID...';
                if(id === 'inp-api-g') el.placeholder = 'AIzaSy...';
            }
        };
        setPlaceholder('inp-api-k', savedKeys.k);
        setPlaceholder('inp-api-c', savedKeys.c);
        setPlaceholder('inp-api-g', savedKeys.g);

        document.getElementById('knd-save-keys').addEventListener('click', () => {
            // Security: Sanitize newlines to prevent HTTP Header Injection
            const sanitizeKey = (val, keyType) => {
                if (val === '') return gameState.apiKeys[keyType]; // Untouched
                if (val.trim() === '') return ''; // Explicitly cleared
                return val.replace(/[\r\n]/g, '').trim();
            };
            gameState.apiKeys.k = sanitizeKey(document.getElementById('inp-api-k').value, 'k');
            gameState.apiKeys.c = sanitizeKey(document.getElementById('inp-api-c').value, 'c');
            gameState.apiKeys.g = sanitizeKey(document.getElementById('inp-api-g').value, 'g');

            setPlaceholder('inp-api-k', gameState.apiKeys.k);
            setPlaceholder('inp-api-c', gameState.apiKeys.c);
            setPlaceholder('inp-api-g', gameState.apiKeys.g);
        ...
        const pingAPI = async (type) => {
            ...
            let targetVal = document.getElementById(`inp-api-${type}`).value;
            targetVal = targetVal === '' ? gameState.apiKeys[type] : targetVal;
            targetVal = (targetVal || '').replace(/[\r\n]/g, '').trim();

            let keyK = document.getElementById('inp-api-k').value;
            keyK = keyK === '' ? gameState.apiKeys.k : keyK;
            keyK = (keyK || '').replace(/[\r\n]/g, '').trim();
